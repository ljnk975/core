...
specify the SSL backend manually.''')
    ...
    def using_openssl(self):
        ...
        if sys.platform == "win32":
            ...
            # ROCKS8 mor libs for removing nasty dependencies
            # -lidn2 -lldap -lzstd
            self.libraries.append('idn2')
            self.libraries.append('ldap')
            self.libraries.append('zstd')
            self.extra_link_args.append(
                    '-L/opt/rocks/lib -lssl -lcrypto')
        self.define_macros.append(('HAVE_CURL_SSL', 1))
